...
import { classNames, Link, toStyleObj, withPrefix } from '../utils';
          ...
          <nav className={classNames('nav', { 'nav--light': is_white_header, 'nav--dark': is_white_header !== true })}>
            <div
              ...
              {...(is_logo_light && _.get(site, 'data.config.logo_light', null)
                ? { 'data-original': withPrefix(_.get(site, 'data.config.logo_light', null)) }
                : null)}
              {...(_.get(site, 'data.config.logo_dark', null)
                ? { 'data-dark': withPrefix(_.get(site, 'data.config.logo_dark', null)) }
                : null)}
            >
                  ...
                  <React.Fragment key={item_idx + '.1'}>
                    <li key={item_idx} className="nav__menu-item">
                      <Link
                        ...
                        className={classNames('nav__menu-item-link', { 'nav__menu-item-link--active': isActive })}
                      >
            ...
            <div className="nav__right">
              ...
              <button className="hamburger button button--transparent">
                <svg width="36" height="24" viewBox="0 0 24 16" xmlns="http://www.w3.org/2000/svg">
                  ...
                  ? {
                      style: toStyleObj(
                        "background-image: url('" +
                          withPrefix(_.get(site, 'data.config.hamburger_background_image', null)) +
                          "')"
                      ),
                    }
                  : null)}
                    ...
                    return (
                      <React.Fragment key={item_idx + '.1'}>
                        <li key={item_idx} className="hamburger__nav-item">
                          <Link
                            href={withPrefix(_.get(item, 'url', null))}
                            className={classNames('hamburger__nav-link', { 'hamburger__nav-link--active': isActive })}
                          >
                            {_.get(item, 'title', null)}
                          </Link>
                        </li>
